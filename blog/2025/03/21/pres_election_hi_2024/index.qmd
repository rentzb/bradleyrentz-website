...
Now that the 2024 U.S. presidential election was a few months ago and we are feeling the brunt of the new administration's actions, it time to revisit those results to see what new information we can learn. 
...
Elections are in part spatial phenomena. We know that for a variety of social and historical reasons, the geographic community one lives in the U.S. tends to be correlated with one's race/ethnicity, socio-economic status, education level, occupation, income, access to government services, health care, and high-quality food, quality of live, and many other factors. Because of that, one's geographic community is also correlated with one's political views. We also know that while each community may tend to have specific political views, those political views are also influenced their neighboring communities and are in turn influenced by their neighbors as well.
...
To take into account the spatial correlations, we'll use a version of CAR models called the [Besag-York-Mollié](https://doi.org/10.1007/BF00116466) model that is frequently used to model incidence rates for diseases. This model includes two set of spatial  correlations. First it models the correlation of a geographic area with all of the areas that border it. Then it models a random effect for each geographic unit so that it captures the unique variation within each unit.  
...
#| include: true
#| message: false
precincts_shape_2024 <- st_read("data/Election_Precincts.shp")
...
Let's update our map to see what the expected support for Trump is. First, we need to generated the predicted values for each precinct using `predicted_draws()` then calculate the median predicted value with `median_hdi()`. Finally we calculate the expected percentage of votes by using the observed total number of votes and compare that percentaged to the observed percentage of votes for Trump.
...
hi_bg_shape_2020 <- st_read("data/hi_bg_2020_bound.shp")
...
It looks like the continuous ACS variables have a fair amount of missingness. We will need to remove or impute those data in our final model later.
...
For our second model, we include all the ACS variables, but for the ones with missing values, we will impute the missing values with the `mi()` in the model itself. To be safe, we will use the `skew_normal()` distribution for the imputation formulae. We also standardized all the predictors in the model to make it easier to compare the results between predictors and easier to set priors for them.
...
To examined the predicted results, we first have to determined the imputed values for those variables that had missing data, then use that data set to calculate the final predicted values for each election district.
